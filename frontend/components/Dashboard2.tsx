...
const normalizePlayer = (player: any): Player => {
  return {
    ...
    WeatherPreference: player.WEATHERCONDITION?.toLowerCase() || ['sunny', 'cloudy', 'rainy'][Math.floor(Math.random() * 3)],
    League: player.LEAGUE || 'Unknown League',
    Country: player.COUNTRY || 'Unknown Country',
    VenuePerformance: {
      'Wembley Stadium': Math.random() * 100,
      'Camp Nou': Math.random() * 100,
      'Old Trafford': Math.random() * 100,
...
const predictBestTeam = (players: Player[], criteria: PredictionCriteria): Player[] => {
  ...
  const scoredPlayers = filteredPlayers.map(player => {
    ...
    const perfScore = (safeNumber(player.Goals) / 10 * 30) + 
                     (safeNumber(player.Assists) / 5 * 20);
    score += perfScore * 0.5;
    
    // Physical attributes (50% weight)
    score += (safeNumber(player.Fitness) / 100 * 20);
    score += (safeNumber(player.Strength) / 100 * 15);
    score += (safeNumber(player.Acceleration) / 100 * 15);
    
    // Weather preference match (20% bonus)
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-gray-50 rounded-lg p-3 text-center hover:bg-blue-50 transition-colors duration-200">
            ...
            <p className="text-xs font-medium text-gray-600 uppercase tracking-wide">Goals</p>
          </div>
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-center hover:bg-green-50 transition-colors duration-200">
            <p className="text-2xl font-bold text-gray-900">{safeNumber(player.Fitness)}</p>
          ...
          {safeNumber(player.Assists) > 0 && (
            <div className="bg-gray-50 rounded-lg p-3 text-center hover:bg-orange-50 transition-colors duration-200">
              <p className="text-lg font-bold text-gray-900">{safeNumber(player.Assists)}</p>
              <p className="text-xs font-medium text-gray-600 uppercase tracking-wide">Assists</p>
            </div>
          )}
          {(safeNumber(player.Strength) > 0 && (
            <div className="bg-gray-50 rounded-lg p-3 text-center hover:bg-red-50 transition-colors duration-200">
              <p className="text-lg font-bold text-gray-900">
                {safeNumber(player.Strength)}
              </p>
              <p className="text-xs font-medium text-gray-600 uppercase tracking-wide">Strength</p>
            </div>
          ))}
        </div>
              ...
              <div className="grid grid-cols-1 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border text-gray-800 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white text-sm"
                  >
                    <option value="Wembley Stadium">Wembley Stadium (England)</option>
                    <option value="Camp Nou">Camp Nou (Spain)</option>
                    <option value="Old Trafford">Old Trafford (England)</option>
...
export default function Dashboard({ onSwitchDashboard }: DashboardProps) {
  ...
  useEffect(() => {
    const fetchPlayers = async () => {
      try {
        ...
        const response = await fetch('http://localhost:8000/football/players/all');
        ...
        // Directly use the data array from response
        const playersData = rawData.data || [];
        console.log("First few players:", playersData.slice(0, 3));
        
        const normalizedPlayers = playersData.map(normalizePlayer);
        console.log("Normalized players:", normalizedPlayers.slice(0, 3));
        
        const uniqueTeams = [...new Set(normalizedPlayers.map((p: { Team: any; }) => p.Team).filter(Boolean))] as string[];
        setTeams(uniqueTeams);
        
        // Remove duplicates based on player No and Name
        const uniquePlayers = normalizedPlayers.filter(
          (player: { No: any; Name: any; }, index: any, self: any[]) => 
        ...
        console.log("Unique players count:", uniquePlayers.length);
  ...
  useEffect(() => {
    console.log("Players state updated with count:", players.length);
  }, [players]);

  useEffect(() => {
    console.log("Filtered players updated with count:", filteredPlayers.length);
  }, [filteredPlayers]);
  ...
  const avgFitness = players.length > 0 
    ? players.reduce((sum, player) => sum + safeNumber(player.Fitness), 0) / players.length 
    : 0;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl font-semibold text-gray-700">Loading Football Data...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-gray-900">Football Dashboard</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                <div className="flex items-center space-x-3 mb-2">
                  ...
                  <h3 className="font-medium text-gray-800">Match Type</h3>
                </div>
                <select
        ...
        {loading && filteredPlayers.length === 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="bg-white rounded-xl shadow-lg p-6 h-64 animate-pulse"></div>
            ))}
          </div>
        ) : (
          <>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="mb-4 md:mb-0">
              <p className="text-gray-600">
                © {new Date().getFullYear()} Football Prediction Pro. All Rights Reserved.
              </p>
            </div>
